<?php
    ...
    if (isset($_POST["add_class"])) {
        if ($tranHandle->register_class($_POST["class_name"], $_POST["class_passcode"], $_SESSION["name"], "empty", $_POST["class_description"])) {
            header("Location: http://localhost/lainCodeMonger_V2/classes_student.php");
        } else {
            echo '<script>';
    ...
    if (isset($_POST["add_member"])) {
        if ($tranHandle->addStudentToCourse($_SESSION["name"], $_POST["class_name"], $_POST["class_passcode"])) {
            echo '<script>';
            echo 'alert("Class registeration succesfull!!!");';
            echo '</script>';
        } else {
            echo '<script>';
            echo 'alert("Class registeration unsuccesfull!!!");';
            echo '</script>';
        }
    }

    //$classesHere = $tranHandle->getClasses($_SESSION["name"]);
    $enrolledClasses = $tranHandle->getStudentsClasses($_SESSION["name"]);
    $remainingClasses = $tranHandle->getRemainderClasses($_SESSION["name"]);
?>
<!Doctype html>
<html lang="en">
    <head>
    ...
    <body>
            ...
            <div class="row">
                <?php
                    foreach($enrolledClasses as $class) {
                        ...
                        echo '          <h5 class="card-title">' . $class .'</h5>';
                        echo '          <a href="http://localhost/lainCodeMonger_V2/dashboard.php?class_name=' . $class . '" class="btn btn-success">Check</a>';
                        echo '      </div>';
                        echo '  </div>';
                        echo '</div>';
                    }
        ...
        <div class="d-flex justify-content-center align-items-center" style="font-size: 32px; font-weight: bold;">
            <p id="textHere" class="text-center">REMAINING CLASSES</p>
        </div>
        <div class="d-flex justify-content-center align-items-center" style="font-size: 32px; font-weight: bold;">
            <hr style="width: 80%;">
        </div>
            ...
            <div class="row">
                <?php
                    foreach($remainingClasses as $class) {
                        ...
                        echo '      <img class="card-img-top" src="images/class.png" alt="Card image cap">';
                        echo '      <div class="card-body">';
                        echo '          <h5 class="card-title" id="classNameFT' . $class . '">' . $class .'</h5>';
                        //echo '          <a href="http://localhost/lainCodeMonger_V2/dashboard.php?class_name=' . $class . '" class="btn btn-success">Enroll</a>';
                        echo '          <button id="addForm' . $class . '" class="btn btn-info">Enroll</button>';
                        echo '      </div>';
                        echo '  </div>';
                        echo '</div>';
                    }
                    echo '<script>';
                    foreach($remainingClasses as $class) {
                        echo 'document.getElementById("addForm' . $class . '").onclick = function() { document.getElementById("_name").value = document.getElementById("classNameFT' . $class . '").textContent; modal.style.display = "block"; }; ';
                        //echo 'document.getElementById("addForm' . $class . '").onclick = function() { console.log("hi"); }';
                    }
                    echo '</script>';
                ?>
            </div>
        ...
        <div id="classForm" class="modal">
            <div class="modal-content">
                ...
                <h2 class="text-center">Enroll Class</h2><br>
                <form action="?" method="POST">
                    ...
                    <div class="d-flex justify-content-center align-items-center" style="font-size: 27px; font-weight: bold;">
                        <button type="submit" name="add_member" class="btn btn-success mt-3">Add</button>
                    </div>
                </form>
            </div>
        </div>
        <script src="js/modalExtra.js"></script>
